...
public class Spellblade_passive extends ItemBase.inherence_handler{
    ...
    public Spellblade_passive(PlayerData data, ItemStack stack,int idx,int duration,double cooltime, Method method) {
        super(data, stack);
        this.data = data;
        this.idx = idx;
        this.stack = stack;
    ...
    private class AttackEventClass extends PlayerAttackEventHandle {
        ...
        public void EventListener(AbstractBaseEvent event) {
            ...
            if(attacker.equals(data) && source.getAttackType() == DamageSource.AttackType.ATTACK ){
                try{
                    DamageSource bonusdamage = DamageSource.class.cast(calcul.invoke(null,attacker,target,source.isRanged()));
